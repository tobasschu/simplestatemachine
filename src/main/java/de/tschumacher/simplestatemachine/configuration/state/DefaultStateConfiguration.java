...
public class DefaultStateConfiguration<State> implements StateConfiguration<State> {

  Map<State, StateChangeHandler> stateConfigurations = new HashMap<State, StateChangeHandler>();

  @Override
  public StateConfiguration<State> permit(State state, StateChangeHandler handler) {
    this.stateConfigurations.put(state, handler);
    return this;
  }

  @Override
  public StateConfiguration<State> permit(State state) {
    return permit(state, null);
  }

  @Override
  public StateChangeHandler handler(State state) {
