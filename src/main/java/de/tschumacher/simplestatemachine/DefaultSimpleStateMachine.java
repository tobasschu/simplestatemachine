...
public class DefaultSimpleStateMachine<State, Context>
  ...
  public Context change(State newState, Context context) {
    final StateChange<State> stateChange = new StateChange<>(this.actualState, newState);
    ...
    final StateChangeHandler<State, Context> handler = actualStateConfig.handler(newState);
    if (handler != null)
