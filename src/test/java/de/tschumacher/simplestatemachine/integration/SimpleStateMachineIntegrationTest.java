...
public class SimpleStateMachineIntegrationTest {
  ...
  public void setUp() {
    ...
    this.config = new DefaultSimpleStateMachineConfig<TestState, String, StringService>();
    this.service =
        new DefaultSimpleStateMachine<TestState, String, StringService>(this.config, actualState);
  }
  ...
  public void simpleTransitionWithContextAndServiceTest() {
    ...
    final StateChangeHandler<String, StringService> handler =
        Mockito.mock(StateChangeHandler.class);
    this.config.configure(TestState.A).permit(TestState.B, handler);
    this.service.change(TestState.B, context, contextService);
  ...
  public void multiTransitionTest() {
    final StateChangeHandler<String, StringService> aToBhandler =
        Mockito.mock(StateChangeHandler.class);
    this.config.configure(TestState.A).permit(TestState.B, aToBhandler);
    final StateChangeHandler<String, StringService> bToChandler =
        Mockito.mock(StateChangeHandler.class);
    ...
    Mockito.verify(aToBhandler).handle(null, null);
    Mockito.verify(bToChandler).handle(null, null);
  }
}
